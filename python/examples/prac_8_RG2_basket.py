...
class ScrewFSM:
    ...
    def __init__(self, sim_dt, nut_height, talbe_height, device, env_idx):
        ...
        self._above_basket_offset = torch.tensor([0.15, 0.1, self._talbe_height + 0.2], dtype=torch.float32, device=self.device) + self._grip_offset
        self._on_basket_offset = torch.tensor([0.15, 0.1, 0.5 * self._talbe_height + 0.05], dtype=torch.float32, device=self.device) + self._grip_offset
...
# create basket asset
basket_file = "urdf/soomac_description/urdf/basket.urdf"
...
for i in range(num_envs):
    ...
    # add basket
    basket_pose.p.x = table_pose.p.x - 0.15  # 0.35  -> 0.5 for place nut
