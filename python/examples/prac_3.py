...
franka_dof_props['stiffness'].fill(1.0)
franka_dof_props['damping'].fill(0.1)
...
def update_franka(t):
    ...
    for i in range(num_envs):
        ...
        pose.p.x = -0.5 + 0.1 * math.sin(1.5 * t - math.pi * float(i) / num_envs)
        pose.p.y = 0.5 + 0.1 * math.cos(2.5 * t - math.pi * float(i) / num_envs)
        pose.p.z = 0.1 * math.cos(1.5 * t - math.pi * float(i) / num_envs)
        pose.r = gymapi.Quat.from_euler_zyx(0.0, 0.0, -math.pi/2)
