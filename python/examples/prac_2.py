...
def control_ik(dpose, damping, j_eef, num_envs):
    ...
    u = (j_eef_T @ torch.inverse(j_eef @ j_eef_T + lmbda) @ dpose).view(num_envs, 7) #댐핑된 최소 자승 문제를 풀고 야코비안의 전치행렬과 역행렬을 곱해 ee 자세변화량 dpose를 계산 
    return u
...
class ScrewFSM:
    ...
    def __init__(self, sim_dt, nut_height, bolt_height, screw_speed, screw_limit_angle, device, env_idx):
        ...
        self._grip_offset = torch.tensor([0, 0, 0.12 + self._nut_height + 0.03], dtype=torch.float32, device=self.device)
        self._lift_offset = torch.tensor([0, 0, 0.15 + self._bolt_height], dtype=torch.float32, device=self.device)
        self._above_bolt_offset = torch.tensor([0, 0, self._bolt_height + 0.03], dtype=torch.float32, device=self.device) + self._grip_offset
        self._on_bolt_offset = torch.tensor([0, 0, 0.8 * self._bolt_height + 0.03], dtype=torch.float32, device=self.device) + self._grip_offset
        self._hand_down_quat = torch.tensor([0, 0.707, 0, 0.707], dtype=torch.float32, device=self.device)
...
# load franka asset
franka_asset_file = "urdf/ur_description/lk_ur5e_with_gripper.urdf"
asset_options = gymapi.AssetOptions()
...
franka_hand_index = franka_link_dict["tool0"]
