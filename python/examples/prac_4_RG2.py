...
franka_lower_limits[6:10] = [-0.30, -0.30, -0.30, -0.50]
franka_upper_limits = franka_dof_props["upper"]
franka_upper_limits[6:10] = [0.30, 0.50, 0.30, 0.30]
...
# grippers open
default_dof_pos[6:] = [0.00, 0.00, 0.00, 0.00]
...
franka_hand_index = franka_link_dict["ur_grip_site"]
...
while not gym.query_viewer_has_closed(viewer):
    ...
    cur_grip_sep_fsm = dof_pos_fsm[:, 7] + dof_pos_fsm[:, 9]
    ...
    #pos_action[:, 8:10] = grip_acts
    pos_action[:, 7] = 1.3*grip_acts[:, 0]  # grip_acts의 첫 번째 값을 7번 인덱스에 할당
    pos_action[:, 9] = -1.3*grip_acts[:, 1]  # grip_acts의 두 번째 값을 9번 인덱스에 할당
